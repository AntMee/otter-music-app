...
describe("parseKugouPlaylistUrl", () => {
  ...
  it("extracts playlist id from PC links", () => {
    expect(
      parseKugouPlaylistUrl(
        "https://www.kugou.com/yy/special/single/6222311.html"
      )
    ).toBe("6222311");
  });
  ...
  it("extracts playlist id from mobile links", () => {
    expect(
      parseKugouPlaylistUrl("https://m.kugou.com/plist/list/6222311")
    ).toBe("6222311");
  });
  ...
  it("extracts global collection id from new songlist links", () => {
    expect(
      parseKugouPlaylistUrl(
        "https://m.kugou.com/songlist/gcid_3z11ahgl0z6z0dc/"
      )
    ).toBe("gcid_3z11ahgl0z6z0dc");
    expect(
      parseKugouPlaylistUrl(
        "https://www.kugou.com/songlist/gcid_3z11ahgl0z6z0dc/"
      )
    ).toBe("gcid_3z11ahgl0z6z0dc");
    expect(
      parseKugouPlaylistUrl(
        "https://m.kugou.com/songlist/?global_collection_id=gcid_3z11ahgl0z6z0dc"
      )
    ).toBe("gcid_3z11ahgl0z6z0dc");
  });
...
describe("buildKugouAndroidSignedUrl", () => {
  it("builds stable Android signed URLs", async () => {
    const url = await buildKugouAndroidSignedUrl(
...
describe("buildKugouGlobalPlaylistSongsUrl", () => {
  it("builds the global collection playlist song endpoint", async () => {
    vi.useFakeTimers();
...
describe("parseKugouPlaylistResponse", () => {
  it("parses wrapped JSON responses", () => {
    const result = parseKugouPlaylistResponse(
...
describe("parseKugouPlaylistTitle", () => {
  it("extracts title from keywords meta", () => {
    const html =
      '<meta name="keywords" content="酷狗正式版,随便听听就好了,歌曲下载" />';
    expect(parseKugouPlaylistTitle(html)).toBe("随便听听就好了");
  });

  it("falls back to page title", () => {
    expect(
      parseKugouPlaylistTitle("<title>我的歌单_精选集_乐库频道_酷狗网</title>")
    ).toBe("我的歌单");
  });
});
...
describe("fetchKugouPlaylistPages", () => {
  it("fetches all pages until total is reached", async () => {
    const detail = await fetchKugouPlaylistPages(
      "6222311",
      async (path: string) => {
